...
// ============================================================
// Disable Dual Clutch (Feature 0x0D01)
// ============================================================
static bool g923_disable_dual_clutch(void) {
#if CFG_TUD_CDC
    printf("g923: disabling dual-clutch mode (feat=0x%02X func=%d)\n",
           G923_FIDX_PEDAL_STATUS, G923_PEDAL_FUNC_SET_MODE);
#endif
    uint8_t params[] = { 0x00 };  // 0x00 = disable dual-clutch
    return hidpp_send_long(G923_FIDX_PEDAL_STATUS, G923_PEDAL_FUNC_SET_MODE,
...
void g923_on_hidpp_response(const uint8_t* report, uint16_t len) {
    ...
    // Move to WAIT_READY and watch for feat=0x11 device ready event.
    // Do NOT send anything yet — device is still completing POST.
    if (g923_init_state == G923_INIT_MOUNTED) {
#if CFG_TUD_CDC
        printf("g923: first IN received (len=%d), waiting for 0x8100 ready (feat=0x%02X)\n",
    ...
    // All states past MOUNTED require a valid HID++ VLong response:
    // report[0] = 0x12  (VLong report ID — NOT stripped by TinyUSB for this device)
    // report[1] = 0xFF  (device index)
    // report[2] = feat_idx
    // report[3] = func|sw_id  (upper nibble = func, lower = sw_id)
    // report[4+] = params
    // Confirmed from pcap analysis.
    ...
    switch (g923_init_state) {
        ...
        case G923_INIT_WAIT_READY:
            // Watch for unsolicited feat=0x11 event (device ready after POST).
            // Confirmed from pcap: fires exactly once after wheel sweep completes.
            // feat=0x0D (pedal status) reports arrive repeatedly during POST — ignore.
            if (feat == G923_FIDX_DEVICE_READY) {  // 0x11
#if CFG_TUD_CDC
                printf("g923: device ready (feat=0x11) — POST complete, starting init\n");
#endif
                g923_init_state = G923_INIT_WAIT_PEDAL_RESET;
                g923_disable_dual_clutch();
            } else {
#if CFG_TUD_CDC
                printf("g923: WAIT_READY ignoring feat=0x%02X (waiting for 0x%02X)\n",
                       feat, G923_FIDX_DEVICE_READY);
#endif
            }
            break;
        ...
        case G923_INIT_WAIT_IROOT:
            // IRoot getFeature response for 0x807A.
            // Confirmed from pcap: runtime index is at report[4].
            // feat=0x00 (IRoot), func=0x0D (sw_id=0xD, func=0).
            // report[4] = 0x12 = G923_FIDX_LED_CTRL.
        ...
        case G923_INIT_WAIT_LED_ENABLE:
            // Ack for LED enable (feat=0x12, func=0x3D confirmed from pcap).
            // HID++ error response: feat=0xFF, func=0xFF, error code in report[4].
            if (feat == 0xFF && func == 0xFF) {
#if CFG_TUD_CDC
            ...
            if (feat != G923_FIDX_LED_CTRL) {
                // Unexpected response — could be a pedal status or position event
                // arriving while we wait. Ignore and keep waiting.
#if CFG_TUD_CDC
                printf("g923: WAIT_LED_ENABLE ignoring feat=0x%02X\n", feat);
#endif
                break;
            }
            g923_led_enabled = true;
#if CFG_TUD_CDC
            printf("g923: LED enable ack (feat=0x%02X func=0x%02X) — sending func6 set\n",
                   feat, func);
#endif
        ...
        case G923_INIT_WAIT_LED_SET:
            // Ack for LED set (feat=0x12, func=0x6D confirmed from pcap).
            if (feat != G923_FIDX_LED_CTRL) {
#if CFG_TUD_CDC
                printf("g923: WAIT_LED_SET ignoring feat=0x%02X\n", feat);
#endif
                break;
            }
#if CFG_TUD_CDC
            printf("g923: LED set ack (feat=0x%02X func=0x%02X) — init complete\n",
                   feat, func);
#endif
        ...
        case G923_INIT_DONE:
            // Post-init unsolicited events:
            // feat=0x0D → pedal status change (dual-clutch toggle etc.)
            // feat=0x0B → FFB status
            // feat=0x01 → wheel position events
            // feat=0x12 → LED acks for ongoing g923_set_leds calls
            // No handling needed here until specific post-init features are added.
            break;
